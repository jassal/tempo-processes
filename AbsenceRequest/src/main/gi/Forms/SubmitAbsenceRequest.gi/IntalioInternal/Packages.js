...
jsx3.lang.Package.definePackage(
  ...
  function(tms) {
    ...
    tms.onGetTaskSuccess = function(event) {
		...
		if(response == null) {
			var msg = Intalio.Internal.Utilities.getErrorGettingTask();
...
jsx3.lang.Package.definePackage(
  ...
  function(attach) {
    ...
    attach.updateCDFNode = function(context) {
        ...
        var href = "<img src='/everteam/gi/files/images/remove.gif' border='0' " + 
...
jsx3.lang.Package.definePackage(
  ...
  function(util) {
    ...
    util.SERVER = SubmitAbsenceRequest;
    util.MAPPING = "IntalioInternal_FormModelMapping_xml";
    util.BUTTON_POSITION="Bottom";
    ...
    util.getRequestParameter = function(key) {
        var uri = new jsx3.net.URI(decodeURIComponent(window.location.href)); // need to decode the URI to resolve special character issue
    ...
    util.validateForm = function() {
        ...
        if (children != null) {
            ...
            for (var i = 0; i < children.length; i++) {
                ...
                // they will throw an exception
                try {
                    if ((element.isType("com.intalio.ria.FileUpload") && element.doValidate() == jsx3.gui.Form.STATEINVALID) || (element.isType("jsx3.gui.RadioButton") && element.doValidate() == jsx3.gui.Form.STATEINVALID) || (element.isType("jsx3.gui.CheckBox") && element.doValidate() == jsx3.gui.Form.STATEINVALID)|| element.getValidationState() == jsx3.gui.Form.STATEINVALID || element.doValidate() == jsx3.gui.Form.STATEINVALID) {
    ...
    util.initApp = function() {
        ...
        /*
         * WF-1554
         * IE : AJAX form fails to adjust itself according to screen resolution
    ...
    util.getErrorMessages = function() {
        ...
        if (window.intalioPreStart) {
                var retval = window.intalioPreStart();
                if (retval != null) {
                    msgArray[msgArray.length++] = retval;
                    return msgArray;
                }
        }
        if (window.intalioPreSave) {
        ...
        if (children != null) {
            ...
            for (var i = 0; i < children.length; i++) {
                var element = children[i];
                var htmlObject = document.createElement('div'); //fix for EDGE-3795, we have to parse the error message
                ...
                try {
				    ...
				    try {
						...
						// Added this to support Custom Validtion
						if ((element.isType("com.intalio.ria.FileUpload") && element.doValidate() == jsx3.gui.Form.STATEINVALID) || (element.isType("jsx3.gui.RadioButton") && element.doValidate() == jsx3.gui.Form.STATEINVALID) || (element.isType("jsx3.gui.CheckBox") && element.doValidate() == jsx3.gui.Form.STATEINVALID)|| element.getValidationState() == jsx3.gui.Form.STATEINVALID || element.doValidate() == jsx3.gui.Form.STATEINVALID) {
							var txt = null;                        
							if (field != null) {
								txt = field.getErrorMessageText();
								if (txt != null) {
									...
									if (txt.length == 0) {
										txt = null;
											...
											msgArray[msgArray.length++] = htmlObject;
									}
								}
							}     
    ...
    util.closeError = function() {
		...
		var msgArray = util.getErrorMessages();
		var matched = false;
		var errorMsg = document.getElementById("IntalioInternal_error_text");
		if((errorMsg.innerHTML.match(util.getErrorIncompleteField())) || (errorMsg.innerHTML.match(util.getErrorIncompleteMatrix())) || (errorMsg.innerHTML.match(util.getErrorUploadingFile()))) {
			matched = true;
			document.getElementById("IntalioInternal_error").style.visibility = "hidden";     
			util.undimForm();
			return;
		}
		errorMsg.innerHTML = errorMsg.innerHTML.replace(/\W/g, ""); //Replace all special characters, otherwise match will fail
		if(msgArray.length > 0) {
			for(var i = 0; i < msgArray.length; i++) {
			msgArray[i] = msgArray[i].innerHTML.replace(/\W/g, ""); //Replace all special characters, otherwise match will fail
				if(errorMsg.innerHTML.match(msgArray[i])) {
					matched = true;
					document.getElementById("IntalioInternal_error").style.visibility = "hidden";     
					util.undimForm();
					return;
	...
	util.getErrorGettingTask = function() {
		var error = util.getProperty("Get Task Error","Error while getting the Task.");
		if (window.IntalioInternal_ErrorGettingTask) {
			error = window.IntalioInternal_ErrorGettingTask;
		}
		return error;
	};
